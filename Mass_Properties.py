...
def Mass_Properties(t,Rocket,Opt):
...
#   - dI_Rdt    Derivative of R. Moment of Inertia [kgm^2/s]
    ...
    if Opt == 'Linear':
        ...
        if t == 0:
            ...
            M = Rocket.rocket_m + Rocket.motor_mass - Rocket.Thrust2dMass_Ratio*Current_Impulse
            dMdt = Rocket.Thrust2dMass_Ratio*Thrust(t,Rocket)
    else:
        raise TypeError("Opt parameter should be Linear or Nonlinear") #error
    ...
    I_L = Rocket.rocket_I + I_L_Casing + I_L_Grain + (Grain_Mass+Rocket.casing_mass)*(Rocket.L-Cm-Rocket.motor_length/2)**2 # I + ... + Steiner
    
    # dI_L/dt:
    dI_L_Grain = dMdt*(Rocket.motor_length**2/12 + (R_e**2+R_i**2)/4)
    
    dI_Ldt = dI_L_Grain+dMdt*(Rocket.L-Cm-Rocket.motor_length/2)**2 + 2*(Grain_Mass+Rocket.casing_mass)*(Rocket.L-Cm-Rocket.motor_length/2)*dCmdt
